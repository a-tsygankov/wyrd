...
test("defaults: classic rules, timers on, telemetry on", () => {
    assert.deepEqual(DEFAULT_SETTINGS, { ruleset: "classic", timers: true, telemetry: true });
...
test("saved settings are restored and unknown values fall back", () => {
    ...
    saveSettings(storage, { ruleset: "teeth", timers: false, telemetry: true });
    assert.deepEqual(loadSettings(storage, new URLSearchParams()), { ruleset: "teeth", timers: false, telemetry: true });
...
test("URL parameters override storage for this visit only", () => {
    const storage = memory();
    saveSettings(storage, { ruleset: "teeth", timers: true, telemetry: true });
    const fromUrl = loadSettings(storage, new URLSearchParams("rules=resolve&timers=off&telemetry=off"));
    assert.deepEqual(fromUrl, { ruleset: "resolve", timers: false, telemetry: false });
    assert.deepEqual(loadSettings(storage, new URLSearchParams()), { ruleset: "teeth", timers: true, telemetry: true }, "storage untouched");

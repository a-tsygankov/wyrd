...
## Layout
- `packages/` TypeScript sources (grammar, content, resolver, simulation) imported by relative path; no build step of their own. Root `tsconfig.json` compiles them plus `apps/web` and `apps/duel-sim` into `dist/`.

...
**Tray (19, `packages/wyrd-content/src/tray.ts`, 2026-09-26):** essences FIRE, WATER, SHADOW, LIFE · targets SELF, ENEMY, GATE · actions SEEK, BIND, WARD, BREAK, MEND, OPEN, CLOSE · modifiers AMPLIFY, WEAKEN, SPLIT, REVERSE, ANCHOR; reactions REFLECT, NULL, SILENCE. The gate is a contested objective (open/closed/broken). Original list: FIRE, SHADOW, SELF, ENEMY, SPELL, GATE · SEEK, BIND, WARD, CLOSE · AMPLIFY, REFLECT, ANCHOR, NULL, SILENCE. The parser supports all 30 registry glyphs and every one has help text (`glyphHelp.ts`); FORCE, ALLY, AREA, PUSH/PULL, DELAY and IF are described but remain outside the resolver (marked `pending`). Spec semantics implemented (grammar spec §10/§12): WEAKEN is AMPLIFY's inverse (magnitude −1, floor 0, a 0 still reaches and seals but dents nothing); REVERSE swaps OPEN↔CLOSE, BREAK↔MEND (BREAK/MEND paired in the registry), else flips AMPLIFY↔WEAKEN, else the spell is invalid; SPLIT makes two branches at full magnitude, one seal per side per encounter, and a single REFLECT turns back one branch. SILENCE strips every modifier. The composer highlights, for every tray glyph, whether it completes a castable spell, keeps one open, or cannot follow (`wyrd-simulation/composer.ts`, pool-backed).

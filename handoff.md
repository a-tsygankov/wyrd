...
## Open items
- Duel engagement options (`docs/duel-engagement-options.md`): decided 2026-09-26 - all four proposals accepted. Shipped as selectable rulesets Classic / Teeth / Pulse / Resolve (Settings). **Still open from the packages**: weather rounds and sudden death (H), progressive telegraph reveal and Scry (G), bot personalities (I), in-match draft (J). The stage (F) shipped 2026-09-26 in SVG/CSS with synthesised sound. Judge each ruleset with Stats → Everyone (per-ruleset seal rate, median commit, matches ended on Resolve).
